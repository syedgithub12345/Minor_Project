...
public class AVL_Tree_4 { 
	...
	float permatch(Node root1, Node root2, ArrayList<Integer> lenArrli, int p1, int p2){
		AVL_Tree_4 tempObj = new AVL_Tree_4();
	...
	public static void main(String[] args) {
        ...
        if (choice == 'Y' || choice == 'y') {
            ...
            numTrees = sc.nextInt();
            nodeArr = new Node[numTrees];

            int j = 0;
            for (int check = 0; check < numTrees; check++) {
                System.out.println("\n\nTree" + (check + 1));
                System.out.println("No. of inputs in the array:");
                int ranArrNum = sc.nextInt();
                int[] arr = new int[ranArrNum];
                System.out.println("Enter " + ranArrNum + " integers:");
                for (int i = 0; i < ranArrNum; i++) {
                    arr[i] = sc.nextInt();
                }
            ...
            System.out.println("Enter the number of trees you want to create with random data:");
            numTrees = sc.nextInt();
            nodeArr = new Node[numTrees];
		...
		while(check<numTrees){
            System.err.println("\n\nTree"+(check+1));
            cnt = 1;
		    AVL_Tree_4 tree = new AVL_Tree_4();
		    System.out.println("No. of inputs in the array:");
		    int ranArrNum = sc.nextInt();
		...
		AVL_Tree_4 tree1 = new AVL_Tree_4();
		char choiceMatch;
		do{
			tree1.treeArr.clear();
