...
public class AVL_Tree_complete { 
	...
	Node getChild(Node root, int key) {
        ...
        if (root == null || root.key == key)
        {
			...
			System.out.println("For Key:"+key+" Recommended elements are:"+recommedArr);
			return root;
		}
	...
	float permatch(Node root1, Node root2, ArrayList<Integer> lenArrli, int p1, int p2){
		AVL_Tree_complete tempObj = new AVL_Tree_complete();
	...
	public static void main(String[] args) {
		...
		while(check<numTrees){
            System.err.println("\n\nTree"+(check+1));
            cnt = 1;
		    AVL_Tree_complete tree = new AVL_Tree_complete();
		...
		AVL_Tree_complete tree1 = new AVL_Tree_complete();
		char choiceMatch;
		do{
			tree1.treeArr.clear();
		...
		char choiceRec;
		do{
			System.out.println("Recommend items between two Users:\nRecommend from Second User to First User\nFirst User :");
			int recU1 = sc.nextInt();
			System.out.println("Second User :");
			int recU2 = sc.nextInt();
			tree1.recommed(nodeArr[recU1-1], nodeArr[recU2-1], numArr, recU1-1, recU2-1);
			System.out.println("Do you want to check again ? : <Y|N>");
